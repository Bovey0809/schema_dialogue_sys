        ...
        for turn in x['turns']:
            ...
            if turn['speaker'] == 'SYSTEM':
                ...
                for frame_id in turn['frames']:
                    ...
                    frame['actions'] = [
                        frame['actions'][k] for k in frame['actions']
                    ]
                    frame['slots'] = [
                        frame['slots'][k] for k in frame['slots']
                    ]
                ...
                for frame_id in turn['frames']:
                    frame = turn['frames'][frame_id]
                    try:
                        frame['state']['active_intent'] = intent_pred[dial_id][
                            uttr][frame_id]
                    except KeyError:
                    ...
                    try:
                        ...
                        for k in request_pred[dial_id][uttr][frame_id]:
                            if float(request_pred[dial_id][uttr][frame_id]
                                     [k]) > 0.9:
                    ...
                    try:
                        for k in nonCate_pred[dial_id][uttr][frame_id]:
                            if float(nonCate_pred[dial_id][uttr][frame_id][k]
                                     [2]) > 1.0:
                                slot_values[k] = [
                                    nonCate_pred[dial_id][uttr][frame_id][k][0]
                                ]
                                p1 = nonCate_pred[dial_id][uttr][frame_id][k][
                                    1][0]
                                p2 = nonCate_pred[dial_id][uttr][frame_id][k][
                                    1][1]
                                slots.append({
                                    "exclusive_end": p2,
                                    "slot": k,
                    ...
                    try:
                        for k in cate_pred[dial_id][uttr][frame_id]:
                            if float(cate_pred[dial_id][uttr][frame_id][k]
                                     [1]) > 0.8 and k not in frame['state'][
                                         'requested_slots']:
                                slot_values[k] = [
                                    cate_pred[dial_id][uttr][frame_id][k][0]
                                ]
                    except KeyError:
                        1
                    ...
                    try:
                        ...
                        for k in slots_cross:
                            if float(slots_cross[k][2]) > 0.9:
                                ...
                                if k not in slot_values and slot_cross in frame_slots_history[
                                        api_cross]:
                                    slot_values[k] = frame_slots_history[
                                        api_cross][slot_cross]
                                    print('cross-copied %s from %s in %s' %
                                          (k, slot_cross, api_cross))
                    except KeyError:
                        1
                    ...
                    try:
                        ...
                        for k in slots_needed:
                            if float(slots_needed[k]) > 0.85:
                                if k not in slot_values and k in sys_slot_provide[
                                        frame_id]:
                                    # if k in sys_slot_provide[frame_id]:
                                    if sys_slot_provide[frame_id][k][2] > 9999:
                                        continue
                                    if sys_slot_provide[frame_id][k][
                                            1] != 'INFORM':
                                        slot_values[k] = sys_slot_provide[
                                            frame_id][k][0]
                                        if schemas[frame_id]['slots'][k][
                                                'is_categorical'] == True:
                                            print(
                                                '1---', dial_id, uttr, k,
                                                sys_slot_provide[frame_id][k]
                                                [0])
                                        if schemas[frame_id]['slots'][k][
