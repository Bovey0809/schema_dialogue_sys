...
class Bert_v1(nn.Module):
    ...
    def forward(self, input_ids, segment_ids, input_mask, uttr_mask, val_mask,
                special_tokens_ids):
...
class InputFeatures(object):
    ...
    def __init__(self, example_id, seq_features, slot_label, slot_tag, start,
                 end, cateVal_idx, dialogue_id, api_id, turn_label,
                 slot_in_usr_his, slot_in_sys_his, slot_request_by_sys,
                 utterance, slot, slot_poss_vals, uttr_tokens):
        ...
        } for _, input_ids, input_mask, segment_ids, uttr_mask, val_mask,
                             special_tokens_ids in seq_features]
...
class DataIterator:
    ...
    def __next__(self):
        ...
        if len(self.data_buffer) == 0:
            self.data_buffer = copy.deepcopy(self.examples[self.idx:self.idx +
                                                           self.k])
            self.idx += self.k
    ...
    def batchify(self, data):
        ...
        special_tokens_ids = torch.tensor(all_special_tokens_ids,
                                          dtype=torch.long)
    ...
    def convert_example_to_feature(self, example):
        ...
        example[12] += [['[PAD]']
                        ] * (self.max_numVals_of_slot - len(example[12]))
        ...
        uttr_tokens_padded = uttr_tokens + ['[PAD]'] * (self.max_uttr_len -
                                                        len(uttr_tokens))
        if example[5]:
        ...
        segment_ids = [0] * (len(uttr_tokens_padded) + 2) + [1] * (
            len(slot_tokens) + len(slot_poss_vals) + 4)
        ...
        for i in range(len(example[12])):
            ...
            tmp_val_att_mask += [0] * (self.max_seq_length -
        ...
        input_mask += [[0] * self.max_seq_length
                       ] * (self.max_seq_length - len(input_mask))
        ...
        if self.show_example:
            ...
            for (tokens, input_ids, input_mask, segment_ids, uttr_mask,
                 val_mask, special_tokens_ids) in seq_features:
...
def accuracy(logits_cls, logits_start, logits_end, logits_cate, logits_req, \
    ...
    neg_examples = (start
                    == (uttr_len - 1).long()) * (end == (uttr_len - 1).long())
    ...
    temp_tag_accuracy[1] = (neg_examples *
                            pred_acc).float().sum().cpu().numpy()
    temp_tag_accuracy[0] = ((~neg_examples) * pred_acc).float().sum().cpu().numpy()
    ...
    temp_cate_accuracy[0] = (cate_preds_acc *
                             (cateVal_idx > 0).float()).cpu().numpy().sum()
    temp_cate_examples[0] = (cateVal_idx > 0).float().cpu().numpy().sum()
    temp_cate_accuracy[1] = (cate_preds_acc *
                             (cateVal_idx == 0).float()).cpu().numpy().sum()
...
if args.history_model_file is not None:
    ...
    logger.info('#Finished loading history model file from %s !' %
                args.history_model_file)

model.to(device)

triu_mat = torch.triu(
    torch.ones(corpus.max_uttr_len, corpus.max_uttr_len).to(device))
