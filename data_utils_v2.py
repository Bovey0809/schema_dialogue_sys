...
class Corpus(object):
	def __init__(self, args, prepare_data_manner=None, data_path='../dstc8-schema-guided-dialogue/', \
		...
		self.tokenizer_bert = BertTokenizer.from_pretrained(args.load_model_dir, \
			do_lower_case=False, cache_dir=args.load_model_dir)

		self.max_numVals_of_slot = 0
		self.max_numIntents = 0

		self.train_schemas = self.load_shemas(self.data_path + 'train0920/schema.json')
		self.dev_schemas = self.load_shemas(self.data_path + 'dev0920/schema.json')

		self.train_dials = self.load_dialogues(self.data_path + 'train0920/', self.train_schemas)
		self.dev_dials = self.load_dialogues(self.data_path + 'dev0920/', self.dev_schemas)
	...
	def get_slotCopy_set(self):
		train_data = self.prepare_copy_slot(self.train_dials, self.train_schemas)
	...
	def get_slotCross_set(self):
		train_data = self.prepare_slot_cross_v1(self.train_dials, self.train_schemas)
	...
	def get_uttrCopy_set(self):
		train_data = self.prepare_copy_uttr(self.train_dials, self.train_schemas)
	...
	def get_slotNotCare_set(self):
		train_data = self.prepare_slots_notCare(self.train_dials, self.train_schemas)
	...
	def get_slotInIntent_set(self):
		train_data = self.prepare_slotsInIntent(self.train_dials, self.train_schemas)
	...
	def get_nonCate_set(self):
		train_data = self.prepare_nonCate_slot(self.train_dials, self.train_schemas)
	...
	def get_Cate_set(self):
		train_data = self.prepare_Cate_slot(self.train_dials, self.train_schemas)
	...
	def get_request_set(self):
		train_data = self.prepare_request_slot(self.train_dials, self.train_schemas)
	...
	def load_dialogues(self, path, schemas):
			...
			for turn in _dial['turns']:
				if turn['speaker'] == 'SYSTEM':
					...
					for frame_id in turn['frames']:
						...
						frame['slots_request'] = copy.deepcopy(sys_request_slot)
						...
						frame['request_slot_key_words'] = set(frame['request_slot_key_words'])
				...
				if turn['speaker'] == 'USER':
					...
					# turn['default_tags'], turn['uttr_tokenized'] = self.default_tags(turn['utterance'])
					turn['default_tags'], turn['uttr_tokenized'] = self.default_tags(turn['uttr_with_sys'])

					turn['state_slots_history'] = copy.deepcopy(state_slots_history)
					turn['frame_slots_history'] = copy.deepcopy(frame_slots_history)
					...
					for frame_id in turn['frames']:
						...
						state_slots = set([key for key in frame['state']['slot_values']])
						...
						for k in list(state_slots - nonCate_slots):
							if k not in frame_slots_history[frame_id] and not _schema['slots'][k]['is_categorical']:
								copy_flag = False
								...
								for frame_id_pre in frame_slots_history:
									...
									for k_pre in frame_slots_history[frame_id_pre]:
										if len(set(frame['state']['slot_values'][k])&set(frame_slots_history[frame_id_pre][k_pre]))>0:
											if k not in slots_cross_api:
												slots_cross_api[k] = {}
											slots_cross_api[k][frame_id_pre, k_pre] = ''
											print('### slot cross-copied from history!')
											print(_dial['file_name'])
							...
							if k not in state_slots_history[frame_id]:
								slots_tmp.append(k)
								continue
							if len(set(frame['state']['slot_values'][k])&set(state_slots_history[frame_id][k]))==0:
								slots_tmp.append(k)
							...
							if _schema['slots'][key]['is_categorical']:
								...
								if key in sys_slot_provide[frame_id]:
									if len(set(slot_values) & set(sys_slot_provide[frame_id][key][0]))>0 \
										...
										print('### categorical slot from sys action!', sys_slot_provide[frame_id][key][1])
										print(_dial['file_name'])
										print(key, frame['state']['slot_values'][key])
										print(turn['utterance'])
								...
								if key in sys_slot_provide[frame_id]:
									if len(set(slot_values) & set(sys_slot_provide[frame_id][key][0]))>0:
										...
										frame['slots_nonCate'][key]['from_sys'] = True
										print('### non_categorical slot from sys action!', sys_slot_provide[frame_id][key][1])
										print(_dial['file_name'])
										print(key, frame['state']['slot_values'][key])
										print(turn['utterance'])
								else:
									print('### missing non_categorical slot, not from usr or sys!')
									print(_dial['file_name'])
									print(key, frame['state']['slot_values'][key])
									print(turn['utterance'])


						state_slots_history[frame_id] = copy.deepcopy(frame['state']['slot_values'])
						for k in state_slots_history[frame_id]:
							frame_slots_history[frame_id][k] = copy.deepcopy(state_slots_history[frame_id][k])

						turn['frames'][frame_id]['cate_slots'] = copy.deepcopy(cate_slots)
						turn['frames'][frame_id]['slots_notCare'] = copy.deepcopy(slots_notCare)
						turn['frames'][frame_id]['slots_cross_api'] = copy.deepcopy(slots_cross_api)
						...
						for slot_name in frame['slots_nonCate']:
							...
							if slot['slot'] not in frame['state']['slot_values']:
							...
							if not tagged_slot_not_in_state:
								slot_value = frame['state']['slot_values'][slot['slot']]
								slot_value = [key.lower() for key in slot_value]


							pos_offset = len('sys : ' +last_sys_turn['utterance'] + ' usr : ')
							...
							if slot_string_ori.lower() not in slot_value and (not tagged_slot_not_in_state):
								print('###tagged_slot_not_same_with_stateValue')
								print(_dial['file_name'])
		...
		print('#total slots copied from sys: ', total_slotsCopy)
		print('#total slots cross-copied: ', total_slotCross)
		print('#total slots cross-copied checked: ', total_slotCross_check)
		return dials
				...
				if _turn['speaker'] == 'SYSTEM':
					...
					for frame_id in _turn['frames']:
						...
						for slot_name in _schema['slots']:
							# if _schema['slots'][slot_name]['is_categorical']:
							# 	continue

							if len(set(slot_name.split('_')) & _frame['slot_key_words']) > 2:
								slot_prior = True
							else:
								slot_prior = False
							...
							slot_categorical = _schema['slots'][slot_name]['is_categorical']

							if slot_name in _frame['slots_nonCate']:
								slot_type = 0
								slot_in_usr_his = False
								slot_in_sys_his = False
								slot_requested_by_sys = False
								tag_list_tokenized = _frame['slots_nonCate'][slot_name]['slot_tags']
								start = _frame['slots_nonCate'][slot_name]['start']
								end = _frame['slots_nonCate'][slot_name]['end']
								cateVal_idx = 0

							elif slot_name in _frame['slots_Cate']:
								slot_val = _frame['actions'][slot_name]['values'][0]
								slot_type = 1
								slot_in_usr_his = False
								slot_in_sys_his = False
								slot_requested_by_sys = False
								if slot_val not in slot_info['possible_values']:
									if slot_val == 'dontcare':
										cateVal_idx = 1
									else:
										print('###Cate_slot_value unseen!')
										print(_dial['file_name'])
										print(slot_name)
										print(_turn['utterance'])
								else:
									cateVal_idx = slot_info['possible_values'].index(slot_val) + 2

								tag_list_tokenized = _turn['default_tags']
								start = len(tag_list_tokenized)
								end = len(tag_list_tokenized)
							elif slot_name in _frame['slots_request']:
								slot_type = 2
								slot_in_usr_his = False
								slt_in_sys_his = False
								slot_requested_by_sys = False
								tag_list_tokenized = _turn['default_tags']
								start = len(tag_list_tokenized)
								end = len(tag_list_tokenized)
				...
				if _turn['speaker'] == 'USER':
					...
					for frame_id in _turn['frames']:
						...
						for slot_name in _schema['slots']:
							...
							if slot_name in _frame['slots_nonCate']:
								if _frame['slots_nonCate'][slot_name]['from_sys']:
									continue
								else:
									slot_type = 0
									slot_in_usr_his = slot_name in _turn['state_slots_history'][frame_id]
									if slot_name in _turn['state_slots_history'][frame_id]:
										print('###noncate_slot already in user history')
										print(_dial['file_name'])
										print(slot_name)
										print(_turn['utterance'])
									# assert slot_name not in _turn['state_slots_history'][frame_id]
									# slot_in_sys_his = False
									slot_in_sys_his = slot_name in _turn['sys_slot_provide'][frame_id]
									if slot_name in _turn['sys_slot_provide'][frame_id]:
										print('###noncate_slot already in system history')
										print(_dial['file_name'])
										print(slot_name)
										print(_turn['utterance'])
									# assert slot_name not in _turn['sys_slot_provide'][frame_id]
									slot_requested_by_sys = slot_name in _turn['sys_request_slot'][frame_id]
									tag_list_tokenized = _frame['slots_nonCate'][slot_name]['slot_tags']
									start = _frame['slots_nonCate'][slot_name]['start']
									end = _frame['slots_nonCate'][slot_name]['end']
									cateVal_idx = 0

							elif slot_name in _frame['cate_slots']:
								slot_val = _frame['state']['slot_values'][slot_name][0]
								if _frame['cate_slots'][slot_name]['from_sys']:
									...
									slot_in_usr_his = slot_name in _turn['state_slots_history'][frame_id]
									if slot_name in _turn['state_slots_history'][frame_id]:
										print('###cate_slot already in user history')
										print(_dial['file_name'])
										print(slot_name)
										print(_turn['utterance'])
									# assert slot_name not in _turn['state_slots_history'][frame_id]
									slot_in_sys_his = slot_name in _turn['sys_slot_provide'][frame_id]
									if slot_name in _turn['sys_slot_provide'][frame_id]:
										print('###cate_slot already in system history')
										print(_dial['file_name'])
										print(slot_name)
										print(_turn['utterance'])
									# assert slot_name not in _turn['sys_slot_provide'][frame_id]
									...
									if slot_val not in slot_info['possible_values']:
										...
										cateVal_idx = slot_info['possible_values'].index(slot_val) + 2
							elif slot_name in _frame['state']['requested_slots']:
								slot_type = 2
								slot_in_usr_his = slot_name in _turn['state_slots_history'][frame_id]
								slot_in_sys_his = slot_name in _turn['sys_slot_provide'][frame_id]
								slot_requested_by_sys = slot_name in _turn['sys_request_slot'][frame_id]
			...
			for _turn in _dial['turns']:
				...
				if _turn['speaker'] == 'USER':
					...
					if frames == last_frames:
						uttr_tokenized = ['keep', '[SEP]'] + _turn['uttr_tokenized']
					else:
						uttr_tokenized = ['jump', '[SEP]'] + _turn['uttr_tokenized']
			...
			for _turn in _dial['turns']:
				...
				if _turn['speaker'] == 'USER':
					...
					for frame_id in _turn['frames']:
						...
						try:
							intent_info = _schema['intents'][_frame['state']['active_intent']]
						except KeyError:
							intent_info = {'required_slots':{}, 'optional_slots':{}, 'description_tokenized':[]}

						api_desp = self.tokenizer_bert.tokenize('api: '+_schema['description'].lower())
						uttr_tokenized = api_desp + ['[SEP]'] + _turn['uttr_tokenized']
						
						for slot_name in _schema['slots']:

							if slot_name in _turn['sys_slot_provide'][frame_id]:
								if _turn['sys_slot_provide'][frame_id][slot_name][1]=='INFORM':
									continue

							if slot_name not in _turn['sys_slot_provide'][frame_id]:
								continue

							slot_required = slot_name in intent_info['required_slots']
							slot_optional = slot_name in intent_info['optional_slots']
							slot_in_sys_his = slot_name in _turn['sys_slot_provide'][frame_id]
							slot_in_usr_his = slot_name in _turn['state_slots_history'][frame_id]
				...
				if _turn['speaker'] == 'USER':
					...
					for frame_id in _turn['frames']:
						...
						for slot_name in _schema['slots']:
							...
							slot_required = slot_name in intent_info['required_slots']
							slot_optional = slot_name in intent_info['optional_slots']
							slot_in_frame_his = slot_name in _turn['frame_slots_history'][frame_id]
							sample_tag = 1
							if len(set(slot_name.split('_')) & _frame['slot_key_words']) > 2:
								slot_prior = True
							else:
							...
							for cross_frame_id in apis:
								if cross_frame_id == frame_id:
									continue
								sample_tag = 1
								for slot_cross in schemas[cross_frame_id]['slots']:
									if slot_cross not in _turn['frame_slots_history'][cross_frame_id]:
										continue
									sample_tag = 1
									slot_in_cross_frame_his = slot_cross in _turn['frame_slots_history'][cross_frame_id]
									slot_cross_desp_tokenized = schemas[cross_frame_id]['slots'][slot_cross]['description_tokenized']
									if slot_name in _frame['slots_cross_api']:
										# print(_frame['slots_cross_api'][slot_name])
										# exit()
										if (cross_frame_id, slot_cross) in _frame['slots_cross_api'][slot_name]:
											sample_tag = 0
											total_slotsCopy += 1
		...
		for dialogue_id in dials:
			...
			for _turn in _dial['turns']:
				if _turn['speaker'] == 'SYSTEM':
					for frame_id in _turn['frames']:
						_frame = _turn['frames'][frame_id]
						if 'NOTIFY_SUCCESS' in [_frame['actions'][act]['act'] for act in _frame['actions']]:
				...
				if _turn['speaker'] == 'USER':
					...
					for frame_id in _turn['frames']:
						...
						api_desp = self.tokenizer_bert.tokenize(frame_id.split('_')[0].lower())
						uttr_tokenized = _turn['uttr_tokenized']
						
						for slot_name in _schema['slots']:
							slot_info = _schema['slots'][slot_name]
							slot_desp_tokenized = slot_info['description_tokenized_simple'] + ['#'] + slot_info['description_tokenized']
							...
							slot_prior = False
							if len(set(slot_name.split('_')) & _frame['slot_key_words']) > 1:
								slot_prior = True
							# else:
							# 	slot_prior = False
							...
							for cross_frame_id in apis:
								if cross_frame_id == frame_id:
									continue
								cross_api_desp = self.tokenizer_bert.tokenize(cross_frame_id.split('_')[0].lower())
								sample_tag = 1
								for slot_cross in schemas[cross_frame_id]['slots']:
									if slot_cross not in _turn['frame_slots_history'][cross_frame_id]:
										continue
									...
									if slot_name in _frame['slots_cross_api']:
										key_words = []
										for x in _frame['slots_cross_api'][slot_name]:
											key_words += x[1].split('_')

										if len(set(slot_cross.split('_')) & set(key_words)) > 0:
	...
	def prepare_copy_uttr(self, dials, schemas):
		...
		for dialogue_id in dials:
			...
			api_desp = self.tokenizer_bert.tokenize(api.lower().replace('_', ' '))
			_data = []
		...
		for dialogue_id in dials:
				...
				if _turn['speaker'] == 'USER':
					...
					for frame_id in _turn['frames']:
						...
						if len(slots) < len(intent2slot[frame_id][intent]):
							print('###Abnormal: ',dialogue_id, _turn['utterance'])
			...
			for api in intent2slot:
				...
				for intent in intent2slot[api]:
					...
					intent_desp = api_desp + ['#'] + intent_info['description_tokenized']
