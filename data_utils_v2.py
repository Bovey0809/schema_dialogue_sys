...
class Corpus(object):
    ...
    def __init__(self,
        ...
        self.train_schemas = self.load_shemas(self.data_path +
                                              'train0920/schema.json')
        self.dev_schemas = self.load_shemas(self.data_path +
                                            'dev0920/schema.json')

        self.train_dials = self.load_dialogues(self.data_path + 'train0920/',
                                               self.train_schemas)
        self.dev_dials = self.load_dialogues(self.data_path + 'dev0920/',
                                             self.dev_schemas)
                    ...
                    for frame_id in _turn['frames']:
                        ...
                        for slot_name in _schema['slots']:
                            ...
                            if slot_name in _frame['slots_nonCate']:
                                ...
                                tag_list_tokenized = _frame['slots_nonCate'][
                                    slot_name]['slot_tags']
                                start = _frame['slots_nonCate'][slot_name][
                                    'start']
                                end = _frame['slots_nonCate'][slot_name]['end']
                                cateVal_idx = 0

                            elif slot_name in _frame['slots_Cate']:
                                slot_val = _frame['actions'][slot_name][
                                    'values'][0]
                                slot_type = 1
                                slot_in_usr_his = False
                                slot_in_sys_his = False
                                slot_requested_by_sys = False
                                if slot_val not in slot_info[
                                        'possible_values']:
                                    ...
                                    cateVal_idx = slot_info[
                                        'possible_values'].index(slot_val) + 2
                        ...
                        for slot_name in _schema['slots']:
                            ...
                            if slot_name in _frame['slots_nonCate']:
                                if _frame['slots_nonCate'][slot_name][
                                    ...
                                    if slot_name in _turn[
                                            'state_slots_history'][frame_id]:
                                        print(
                                            '###noncate_slot already in user history'
                                        )
                                        print(_dial['file_name'])
                                        print(slot_name)
                                        print(_turn['utterance'])
                                    ...
                                    if slot_name in _turn['sys_slot_provide'][
                                            frame_id]:
                                        print(
                                            '###noncate_slot already in system history'
                                        )
                                        print(_dial['file_name'])
                                        print(slot_name)
                                        print(_turn['utterance'])
                                    ...
                                    slot_requested_by_sys = slot_name in _turn[
                                        'sys_request_slot'][frame_id]
                                    tag_list_tokenized = _frame[
                                        'slots_nonCate'][slot_name][
                                            'slot_tags']
                                    start = _frame['slots_nonCate'][slot_name][
                                        'start']
                                    end = _frame['slots_nonCate'][slot_name][
                                        'end']
                                    cateVal_idx = 0

                            elif slot_name in _frame['cate_slots']:
                                slot_val = _frame['state']['slot_values'][
                                    slot_name][0]
                                if _frame['cate_slots'][slot_name]['from_sys']:
                                    ...
                                    if slot_name in _turn[
                                            'state_slots_history'][frame_id]:
                                        print(
                                            '###cate_slot already in user history'
                                        )
                                        print(_dial['file_name'])
                                        print(slot_name)
                                        print(_turn['utterance'])
                                    ...
                                    if slot_name in _turn['sys_slot_provide'][
                                            frame_id]:
                                        print(
                                            '###cate_slot already in system history'
                                        )
                                        print(_dial['file_name'])
                                        print(slot_name)
                                        print(_turn['utterance'])
                                    ...
                                    if slot_val not in slot_info[
                                        ...
                                        cateVal_idx = slot_info[
                                            'possible_values'].index(
                                                slot_val) + 2
                            elif slot_name in _frame['state'][
                                    'requested_slots']:
                                slot_type = 2
                                slot_in_usr_his = slot_name in _turn[
                                    'state_slots_history'][frame_id]
                ...
                if _turn['speaker'] == 'USER':
                    ...
                    for frame_id in _turn['frames']:
                        ...
                        if last_intent[frame_id] in intents[frame_id]:
                            last_intent_tag[intents[frame_id].index(
                                last_intent[frame_id])] = 1
