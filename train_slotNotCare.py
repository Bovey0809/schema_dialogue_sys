...
from pytorch_pretrained_bert.modeling import BertForSlotNotCare
from pytorch_pretrained_bert.optimization import BertAdam
from pytorch_pretrained_bert.file_utils import PYTORCH_PRETRAINED_BERT_CACHE
from nltk.corpus import stopwords

from data_utils_v2 import Corpus
...
class Bert_v1(nn.Module):
	def __init__(self, ninput, nhidden, n_layer=1, dropout=0, device=None):
		...
		self.bert = BertForSlotNotCare.from_pretrained(args.load_model_dir, \
												cache_dir=args.load_model_dir)

	def forward(self, input_ids, segment_ids, input_mask):

		bsz = input_ids.size(0)
		logits = self.bert(input_ids, segment_ids, input_mask)
...
class InputFeatures(object):
	def __init__(self,
		...
		self.seq_features = [
			...
			for _, input_ids, input_mask, segment_ids in seq_features
		]
...
class DataIterator:
	...
	def filt_data(self):
		...
		for i in list(range(len(self.examples)))[::-1]:
			...
			if example[-1] != 0 and not self.eval:
	...
	def batchify(self, data):
		...
		return [input_ids, input_mask, segment_ids, slot_label, \
			   dialogue_id, turn_label, utterance, uttr_tokens, slot]


	def convert_example_to_feature(self, example):
		seq_features = []
		dialogue_id = example[0]
		turn_label = example[1]
		utterance = example[2]
		slot = example[3]

		uttr_tokens = example[4][-self.max_uttr_len:]
		slot_tokens = example[5]
		...
		if self.show_example:
			...
			for (tokens, input_ids, input_mask, segment_ids) in seq_features:
...
def train(train_features, epoch, global_step):
	...
	for step, batch in enumerate(train_features):
		batch[:4] = tuple(t.to(device) for t in batch[:4])
		input_ids, input_mask, segment_ids, slot_label = batch[:4]
		bsz = input_ids.size(0)
...
def evaluate(eval_features):
	...
	for sample_idx, batch in enumerate(eval_features):
		batch[:4] = tuple(t.to(device) for t in batch[:4])
		input_ids, input_mask, segment_ids, slot_label, \
		dialogue_id, turn_label, utterance, uttr_tokens, slot = batch
	...
	total_results = (total_dialogue_id, total_turn_label, total_utterance, total_uttr_tokens, \
					total_slots, total_score, total_ground, total_preds)
