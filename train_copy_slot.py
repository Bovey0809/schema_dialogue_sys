...
from pytorch_pretrained_bert.modeling import BertForSlotCopy
...
class Bert_v1(nn.Module):
	def __init__(self, ninput, nhidden, n_layer=1, dropout=0, device=None):
		...
		self.bert = BertForSlotCopy.from_pretrained(args.load_model_dir, \
...
class DataIterator:
	...
	def filt_data(self):
		...
		for i in list(range(len(self.examples)))[::-1]:
			...
			if example[-2] != 0 and not example[-1] and not self.eval:
	...
	def batchify(self, data):
		...
		return [input_ids, input_mask, segment_ids, special_tokens_ids, slot_label, \
			   dialogue_id, api_id, turn_label, utterance, uttr_tokens, slot, slot_in_sys_his, \
			   slot_in_usr_his, slot_required, slot_optional]
	...
	def convert_example_to_feature(self, example):
		...
		slot_in_sys_his = example[5]
		slot_in_usr_his = example[6]
		slot_required = example[7]
		slot_optional = example[8]
		utterance = example[3]
		slot = example[4]

		uttr_tokens = example[9][-self.max_uttr_len:]
		slot_tokens = example[10]
		...
		if slot_in_usr_his:
			special_tokens += ['#', 'in', 'user', 'history']
			special_tokens_ids += 2
		if slot_required:
			special_tokens += ['#', 'required', 'in', 'intent']
			special_tokens_ids += 4
		if slot_optional:
			special_tokens += ['#', 'optional', 'in', 'intent']
		...
		segment_ids = [0] * (len(uttr_tokens_padded) + 2) + [1] * (len(slot_tokens) + 1)
		...
		input_mask += [slot_att_mask] * (len(special_tokens))
...
def evaluate(eval_features):
	...
	for sample_idx, batch in enumerate(eval_features):
		...
		dialogue_id, api_id, turn_label, utterance, uttr_tokens, slot, slot_in_sys_his,\
							slot_in_usr_his, slot_required, slot_optional = batch
	...
	total_results = (total_dialogue_id, total_api_id, total_turn_label, total_utterance, total_uttr_tokens, \
					total_slots, total_slot_in_sys_his, total_slot_in_usr_his, \
					total_slot_required, total_slot_optional,\
...
# print(len(corpus.train_dials), len(corpus.dev_dials))
# corpus.sample_dev(p=0.0)
# pkl.dump(corpus, open('corpus_sample0p0Dev_1009.pkl', 'wb'))
# print(len(corpus.train_dials), len(corpus.dev_dials))
# exit()

# corpus = pkl.load(open('corpus_sample0p99Dev.pkl', 'rb'))
corpus.max_uttr_len = 96
train_data, dev_data, test_data = corpus.get_slotCopy_set()
