...
def charIdx2wordIdx_v1(utterance):
	...
	for idx, w in enumerate(uttr_words):
		...
		if len(words) > 1:
			offset = 0
			for _w in words:
				wordTokenized2char[len(wordTokenized2char)] = word2char[idx][offset:offset+len(_w)]
				offset += len(_w)
	...
	for idx, w in enumerate(uttr_words_tokenized):
		words = tokenizer_bert.tokenize(w)
		for _ in words:
			BertTokenized2char[len(BertTokenized2char)] = wordTokenized2char[idx]
		word_to_BertTokenized[idx] = list(range(len(uttr_words_BertTokenized), \
			len(uttr_words_BertTokenized)+len(words)))
		uttr_words_BertTokenized += words
		...
		for turn in _dial['turns']:
			if turn['speaker'] == 'SYSTEM':	
				...
				uttr_with_sys = 'sys : ' +last_sys_turn['utterance'] + ' usr : ' \
												+ turn['utterance']
				uttr_inp = default_tokenize(uttr_with_sys)[-(96-1):] + ['null']
				...
				_dial[turn['utterance']]['tokenIdx2charIdx'] = charIdx2wordIdx_v1(uttr_with_sys)
				_dial[turn['utterance']]['offset'] = len('sys : ' +last_sys_turn['utterance'] + ' usr : ')
...
def process_nonCate_span(slots_categorical):
	...
	for i in range(len(total_eval[3])):
		...
		if total_eval[-8][i] == 0:
			if total_eval[-6][i][0] == total_eval[-5][i][0] and total_eval[-6][i][1] == total_eval[-5][i][1]:
	...
	for i in tp_samples:
		...
		tag_truth = inverse_tag(total_eval[4][i], uttr_tag[i], total_eval[-6][i])
		tag_pred = inverse_tag(total_eval[4][i], uttr_tag[i], total_eval[-5][i])
	...
	F1_micro = 2*precision_micro*recall_micro/(precision_micro+recall_micro+1e-05)
	...
	F1_macro = 2*precision_macro*recall_macro/(precision_macro+recall_macro+1e-05)
	...
	for i in range(len(total_eval[3])):
		...
		token_offset = len(dev_dials[dial_id][uttr]['tokenIdx2charIdx']) + 1 - len(total_eval[4][i])

		if total_eval[-5][i][0]+token_offset == len(dev_dials[dial_id][uttr]['tokenIdx2charIdx']):
			assert total_eval[4][i][total_eval[-5][i][0]] == 'null'
			continue

		if total_eval[-5][i][1]+token_offset == len(dev_dials[dial_id][uttr]['tokenIdx2charIdx']):
			assert total_eval[4][i][total_eval[-5][i][1]] == 'null'
			continue

		p1 = dev_dials[dial_id][uttr]['tokenIdx2charIdx'][total_eval[-5][i][0]+token_offset][0]
		p2 = dev_dials[dial_id][uttr]['tokenIdx2charIdx'][total_eval[-5][i][1]+token_offset][-1]
		...
		if tokenizer_bert.tokenize(str_ori)!= tokenizer_bert.tokenize(tag_pred):
		...
		if str_ori not in exception[dial_id][uttr_inp]:
			results[dial_id][uttr_inp][api][slot] = [str_ori, (p1, p2+1), '%.4f'%(total_eval[-10][i])]
			exception[dial_id][uttr_inp][str_ori] = [api, slot, total_eval[-10][i]]
		else:
...
def process_Cate(slots_categorical):
	...
	for i in range(len(total_eval[3])):
		...
		if val_pred != "null" and val_pred != '[PAD]':
			...
			if api not in results[dial_id][uttr_inp]:
				results[dial_id][uttr_inp][api] = {}
			results[dial_id][uttr_inp][api][slot] = [val_pred, '%.4f'%(total_eval[-9][i])]
...
total_eval = list(pkl.load(open('../detailed_results/2019-10-11_13_04_02_total_eval_results.pkl', 'rb')))
