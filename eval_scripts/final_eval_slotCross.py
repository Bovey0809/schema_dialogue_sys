...
def process_slotCopy_pred(slots_categorical):
	...
	for i in range(len(total_eval[0])):
		...
		slot_name = total_eval[6][i]

		if total_eval[3][i] == 'SYSTEM':
			continue
	...
	for i in tp_samples:
		...
		print('#tp samples: ', 'in train: %s'%str(total_eval[6][i] in slots_api_train), \
				related_api_in_train, related_files_in_train, total_eval[8][i], total_eval[9][i], \
				 total_eval[10][i], total_eval[11][i], total_eval[12][i], '\n', total_eval[0][i], total_eval[1][i], total_eval[2][i],\
					total_eval[4][i], total_eval[6][i], total_eval[7][i], \
					total_eval[-2][i], total_eval[-1][i], '%.4f'%(total_eval[-3][i]), '\n')
	...
	for i in fn_samples:
		...
		print('#fn samples: ', 'in train: %s'%str(total_eval[6][i] in slots_api_train), \
				related_api_in_train, related_files_in_train, total_eval[8][i], total_eval[9][i], \
				 total_eval[10][i], total_eval[11][i], total_eval[12][i], '\n', total_eval[0][i], total_eval[1][i], total_eval[2][i],\
					total_eval[4][i], total_eval[6][i], total_eval[7][i], \
					total_eval[-2][i], total_eval[-1][i], '%.4f'%(total_eval[-3][i]), '\n')
	...
	for i in fp_samples:
		...
		print('#fp samples: ', 'in train: %s'%str(total_eval[6][i] in slots_api_train), \
				related_api_in_train, related_files_in_train, total_eval[8][i], total_eval[9][i], \
				 total_eval[10][i], total_eval[11][i], total_eval[12][i], '\n', total_eval[0][i], total_eval[1][i], total_eval[2][i],\
					total_eval[4][i], total_eval[6][i], total_eval[7][i], \
					total_eval[-2][i], total_eval[-1][i], '%.4f'%(total_eval[-3][i]), '\n')
	...
	F1_micro = 2*precision_micro*recall_micro/(precision_micro+recall_micro+0.00000001)
	...
	F1_macro = 2*precision_macro*recall_macro/(precision_macro+recall_macro+0.0000001)
	...
	for i in range(len(total_eval[3])):
		slot = total_eval[6][i]
		slot_cross = total_eval[7][i]
		dial_id = total_eval[0][i]
		api = total_eval[1][i]
		api_cross = total_eval[2][i]
		uttr = total_eval[4][i]


		uttr_inp = ' '.join(total_eval[5][i])
		...
		if slot not in results[dial_id][uttr_inp][api]:
			results[dial_id][uttr_inp][api][slot] = [api_cross, slot_cross, '%.4f'%(total_eval[-3][i])]
		else:
			if total_eval[-3][i] > float(results[dial_id][uttr_inp][api][slot][2]):
				results[dial_id][uttr_inp][api][slot] = [api_cross, slot_cross, '%.4f'%(total_eval[-3][i])]
			

	with open("slotCross_pred_0p99sampleDev_testGround_onTest.json","w") as dump_f:
...
total_eval = list(pkl.load(open('../detailed_results/2019-10-11_23_16_58_total_eval_results.pkl', 'rb')))
