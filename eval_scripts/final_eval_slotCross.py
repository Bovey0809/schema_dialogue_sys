...
def process_slotCopy_pred(slots_categorical):
    ...
    for i in range(len(total_eval[3])):
        ...
        if slot not in results[dial_id][uttr_inp][api]:
            ...
            if total_eval[-3][i] > float(
                    results[dial_id][uttr_inp][api][slot][2]):
                results[dial_id][uttr_inp][api][slot] = [
                    api_cross, slot_cross,
                    '%.4f' % (total_eval[-3][i])
                ]

    with open("slotCross_pred_0p99sampleDev_testGround_onTest.json",
              "w") as dump_f:
        json.dump(results,
                  dump_f,
                  sort_keys=True,
...
total_eval = list(
    pkl.load(
        open('../detailed_results/2019-10-11_23_16_58_total_eval_results.pkl',
             'rb')))
