...
tokenizer_bert = BertTokenizer.from_pretrained(
    'bert-base-cased',
    do_lower_case=False,
    cache_dir='/DSTC8-SGDST/pre_trained_models_base_cased')
...
def default_tokenize(utterance):
    ...
    for idx, w in enumerate(uttr_words_tokenized):
        ...
        word_to_BertTokenized[idx] = list(
            range(len(uttr_words_BertTokenized),
    ...
    for item in raw['turns']:
        ...
        for frame in item['frames']:
            if temp_turn['speaker'] == 'SYSTEM':
                ...
                slots_sys_his[frame['service']] = set([
                    key
                    for key in temp_turn['frames'][frame['service']]['actions']
                ])
...
def load_dev_dial(path):
    ...
    for file in file_list:
        if 'dialogue' in file:
            ...
            data += _data
            fp.close()
    print('#number of dev dials:', len(data))
    ...
    for dialogue_id in dials:
        ...
        for turn in _dial['turns']:
            if turn['speaker'] == 'SYSTEM':
                ...
                uttr_with_sys = 'sys : ' + last_sys_turn[
                    'utterance'] + ' usr : ' + turn['utterance']
...
def process_intent():
    ...
    for i in fn_samples:
        ...
        print(
            'Error samples: ',
            'intent in Train: %s' % (total_eval[4][i][0] in intents_api_train),
...
def process_intent_infer():
    ...
    for i in range(len(total_eval[0])):
        ...
        if 'jump [SEP]' in ' '.join(
                total_eval[4][i]) or 'keep [SEP]' in ' '.join(
                    total_eval[4][i]):
        ...
        tmp_results[dial_id][turn_id][api_id][last_intent_label] = (
            total_eval[0][i], total_eval[1][i], total_eval[2][i], uttr_inp,
            total_eval[4][i], total_eval[5][i], total_eval[6][i],
    ...
    for dial_id in tmp_results:
        for i in range(len(tmp_results[dial_id])):
            for api in tmp_results[dial_id][i]:
                ...
                try:
                    last_intent_label = tmp_results[dial_id][
                        i - 1][api]['intent_label']
                except KeyError:
                    last_intent_label = tmp_results[dial_id][i][api][0][
                        5].index('none')

                tmp_results[dial_id][i][api]['intent_label'] = tmp_results[
                    dial_id][i][api][last_intent_label][-1]

                tmp_results[dial_id][i][api]['pred'] = intent[
                    tmp_results[dial_id][i][api]['intent_label']]

                if tmp_results[dial_id][i][api]['intent_label'] == tmp_results[
                        dial_id][i][api][last_intent_label][-2]:
                    right += 1
                else:
                    wrong += 1
                    ...
                    print(
                        'Error samples: ', 'intent in Train: %s' %
                        (intent[0] in intents_api_train), dial_id, utterance,
                        api, ground, pred)

    print('right:%d, wrong:%d, acc:%.4f' % (right, wrong, 1. * right /
                                            (right + wrong)))
...
total_eval = list(
    pkl.load(
        open(
            '/DSTC8-SGDST/detailed_results/2022-03-29_12_10_09_total_eval_results.pkl',
            'rb')))
